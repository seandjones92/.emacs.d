...
selections. It's a much better way to interact with Emacs. I'm working
on a way to split this out into smaller functions so I can explain
this config one chunk at a time.
#+BEGIN_SRC emacs-lisp
  (defun my-helm-setup ()
    (require 'helm-config)
