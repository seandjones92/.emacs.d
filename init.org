...
If this is the first launch of Emacs and we have access to the
internet ensure that =use-package= is installed. If we do not have
...
*** Configuration
Now that we know =use-package= is installed we can initialize it for
use with the remaining packages.
#+BEGIN_SRC emacs-lisp
  (eval-when-compile
    (require 'use-package))
#+END_SRC
